...
function buildAsistenciasFilter({ distrito, area, servicio, desde, hasta, q, documento }, startIndex) {
  const conditions = [];
  const params = [];
  let i = startIndex;

  if (documento) {
    conditions.push(`a.nro_doc = $${i++}`);
...
async function listAsistencias({ distrito, area, servicio, desde, hasta, q, documento, offset = 0, limit = 100 }) {
  const { where, params, nextIndex } = buildAsistenciasFilter({ distrito, area, servicio, desde, hasta, q, documento }, 1);
  const limitParam = nextIndex;
  const offsetParam = nextIndex + 1;

...
// `preciosPermitidos` es el mapa de niveles de precio habilitados para el
// rol que hace la operación (null = sin restricción, ej. Admin). Solo se
// aplica a ítems NUEVOS (sin id): uno que ya estaba guardado con un precio
// que el rol actual ya no ve, se puede seguir editando en cantidad/monto sin
// que el guardado lo rechace.
function normalizarItems(items, preciosPermitidos) {
  ...
  return items.map((item, indice) => {
    ...
    const id = item.id !== undefined && item.id !== null && item.id !== '' ? Number(item.id) : null;
    const tipoPrecio = String(item.tipo_precio || '').trim() || null;

    if (id === null && tipoPrecio && preciosPermitidos && !preciosPermitidos[tipoPrecio]) {
      throw new Error(`No tenés permiso para usar el precio "${tipoPrecio}" en "${nombre}"`);
    }
...
async function createCotizacion(input, preciosPermitidos) {
  ...
  const items = normalizarItems(input.items, preciosPermitidos);
...
async function updateCotizacion(id, input, preciosPermitidos) {
  const cabecera = normalizarCabecera(input);
  const items = normalizarItems(input.items, preciosPermitidos);

...
async function deleteByDocumento(documento) {
  const target = normalizeDocumento(documento);
  if (!target) throw new Error('DOCUMENTO es requerido');
  const { rowCount } = await pool.query('DELETE FROM clientes WHERE documento = $1', [target]);
  return rowCount > 0;
}

module.exports = { lookupByDocumento, saveCliente, deleteByDocumento };
